...
module tb_Top();
    ...
    always #5.00  clk_100M = ~clk_100M; 
    always #10.00 clk_50M  = ~clk_50M;  
    always #12.50 clk_40M  = ~clk_40M;  
    always #20.00 clk_25M  = ~clk_25M;  
    always #33.33 clk_15M  = ~clk_15M;  
    ...
    task ler_sensor_e_verificar(input logic [1:0] id_alvo, input string nome_sensor);
        begin
            ...
            // Sincroniza e envia comando
            @(posedge clk_100M);
            reg_id = id_alvo;
            start = 1'b1;
            
            // Espera o Master confirmar que começou a trabalhar
            wait(ready == 1'b0);
            
            // Agora podemos desligar o comando start com segurança
            @(posedge clk_100M);
            start = 1'b0;

            // Espia o valor que será salvo
            case(id_alvo)
            ...
            // Espera o Master fazer todas as leituras SPI e voltar a ficar livre
            wait(ready == 1'b1);
            ...
            // Checagem Automática usando a saída oficial do TOP (ram_data_o)
